...
const RideItem: React.FC<RideItemProps> = ({ data }) => {
	...
	const rideStatusColor = {
		color: ride.status === -1 ? 'red' : ride.status === 0 ? 'dodgerblue' : 'greenyellow',
	};

	return (
		<Flex style={styles.item}>
			<HStack alignItems={"center"} space={2}>
				<Image style={styles.image} source={Images.Seat4Car} size={"md"} alt="img" />
				<VStack space={2}>
					<HStack>
						<Text >Customer: </Text>
					...
					<HStack style={styles.location}>
						<Text >From: </Text>
						<Text style={styles.boldText}>{truncateString(ride.pickup, 100)}</Text>
					</HStack>
					<HStack style={styles.location}>
						<Text >To: </Text>
						<Text style={styles.boldText}>{truncateString(ride.dropoff, 100)}</Text>
					</HStack>
					<HStack>
						<Text >Time: </Text>
					...
					<HStack>
						<Text >Status: </Text>
						<Text style={{ color: ride.status === -1 ? 'red' : ride.status === 0 ? 'dodgerblue' : 'greenyellow' }}>
